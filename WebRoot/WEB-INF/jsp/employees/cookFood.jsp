            ...
            <div style="width:99%; margin:auto">
				...
				<table class="table table-bordered table-striped table-hover">
					<tbody>
         			...
         			<c:forEach items="${orderUsers}" var="o">
						<tr align="left">
							<td>${o.username}</td>
							<td>${o.tableid}</td>
							<td>${o.ordertime}</td>
							<td>${o.state==1?"已有厨师接单":"暂无厨师接单"}</td>
							<td>￥${o.price}</td>
							<c:choose>
								<c:when test="${o.state==1}">
									<td>
										<a onclick="javascript:return toget()" href="toOrderUpd.action?id=${o.id }" class="btn btn-info" style="width:60px;">接单</a>							
									</td>
								</c:when>
								<c:otherwise>
									<td>
										<a href="#" class="btn btn-info" style="width:60px;">已被接单</a>							
									</td>
								</c:otherwise>
							</c:choose>
							<td>
								<a href="fooddel.action?id=${f.id }" class="btn btn-info" style="width:80px;">查看顾客详情</a>
							</td>
         				</tr>
         			</c:forEach>
					</tbody>

					...
					<tbody>
         			...
         			<c:forEach items="${detailsFoods}" var="d">
						<tr align="left">
							...
							<td>${d.num}</td>
							<c:choose>	
								<c:when test="${d.price==1.0}">			
									<td>
										<a href="${pageContext.request.contextPath}/order/orderDetailsUpd.action?id=${d.id}" class="btn btn-info" style="width:80px;">做菜</a>
									</td>
								</c:when>	
								<c:otherwise>
									<td>
										<a href="#" class="btn btn-info" style="width:80px;">已做完</a>
									</td>
								</c:otherwise>	
							</c:choose>
         				</tr>
